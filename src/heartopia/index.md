---
title: Heartopia Daily Guide
layout: default
description: Daily drops, community beats, and latest Heartopia pulse.
lastModified: 2026-03-01 12:00:00
---

## Resources today (1 Mar 2026)
- **Fluorite:** Plot 8 on both Sea Server and America/Global Server! Glyph Hub's morning post confirms dual-server consistency—no need to check which server you're on today. Head straight to Plot 8 and start mining.
- **Oak Timber:** The Wandering Oak is at Plot 8 on Sea Server and Plot 6 on America/Global today. Check your server settings before heading out to save travel time.
- **Pink Bubbles:** Glyph Hub's weekly Pink Bubble map (Feb 28 - Mar 7) is still active! Use the 4-image gallery below to plan your collection route—15 locations mapped across all servers.
- **Weather:** Clear skies continue—ideal conditions for resource collection runs.
- **Winter Event:** Lunar Nook reports Week 5 of the Winter Event is live! The Penguin story concludes with new material collection missions featuring radish, winter food, and winter bird cards. Don't miss the Sea Lantern event at the beach!

## Fluorite & Oak check (Mar 1)
![Glyph Hub's Mar 1 coordinate map](/assets/heartopia-glyphhub-01mar.jpg)
*Glyph Hub's Mar 1 visual shows dual-server fluorite and oak locations.*

### All Servers
| Resource | Sea Server (UTC+7) | America/Global (UTC-5) |
|----------|-------------------|----------------------|
| 💎 Fluorite | Plot 8 | Plot 8 |
| 🪵 Roaming Oak | Plot 8 | Plot 6 |

- Both servers have fluorite at Plot 8 today—unusual consistency that makes collection runs simpler.
- Glyph Hub's coordinated post maps both server sets, confirming Plot 8 fluorite across the board.
- Oak locations differ by server: Plot 8 on Sea (main housing cluster) vs Plot 6 on America/Global (quieter residential area).

## Pink Bubble Locations (Feb 28 - Mar 7)
![Pink Bubble gallery - Server 1](/assets/pink-bubble-01mar-1.jpg)
![Pink Bubble gallery - Server 2](/assets/pink-bubble-01mar-2.jpg)
![Pink Bubble gallery - Server 3](/assets/pink-bubble-01mar-3.jpg)
![Pink Bubble gallery - Server 4](/assets/pink-bubble-01mar-4.jpg)

- All 15 pink bubble locations mapped for February 28 to March 7, 2026.
- Multiple server variations shown—find your server's pattern in the gallery.
- Pink bubble furniture rewards are randomized per player—pop them all for a chance at bathroom sets!
- Pair bubble collection with fluorite/oak runs for maximum efficiency.

## Winter Event Week 5 - Final Chapter!
The Winter Event concludes with Week 5:

- **Penguin Story Finale:** Complete the material collection mission with radish, winter food, and winter bird cards.
- **Sea Lantern Event:** Head to the beach for special lantern-related activities.
- Event is time-limited—finish story missions before the event wraps up!
- Lunar Nook notes this is the final week, so grab any exclusive rewards while you can.

## Yesterday's recap (Feb 28)
- **Fluorite:** Plot 8 on Sea Server, Onsen Mountain on America/Global
- **Roaming Oak:** Plot 6 on Sea Server, Plot 8 on America/Global
- 🌠 **Meteor Shower:** Lunar Nook reported a meteor shower on Feb 28 evening with clear viewing conditions.
- Doris (Starry) was available at the bus stop until 24:00—meteor shard collection event active.
- Glyph Hub posted comprehensive Doris & Meteor Shards location maps for Fisherman Village.

## Energy restoration guide
Heartopia uses standard food items to restore energy. Here's what works:

| Food Item | Energy Restored | Source |
|------------|-----------------|--------|
| 🍰 Cake | +40 | Cooking, Bakery |
| ☕ Coffee | +35 | Cafe |
| 🍜 Noodles | +30 | Cooking |
| 🥧 Pie | +25 | Cooking |
| 🍪 Cookie | +18 | Cooking |
| 🍞 Bread | +15 | Cooking, Bakery |
| 🥗 Salad | +20 | Cooking |
| 🍓 Strawberry | +12 | Gardening |
| 🥕 Carrot | +8 | Gardening |
| 🍎 Apple | +10 | Foraging, Blanc's Shop |

**Free recovery methods:**
- 💤 Sleep in bed: Fully restores stamina
- 🛁 Hot springs: +1 energy every 10 seconds (Onsen Mountain, home bathtub)
- Green Arrow bubbles: Energy refill buffs from prize bubbles
- Friendship buff: One-time daily energy restore from high-friendship residents

## Codes & vault intel
Active codes (valid until March 31, 2026):

- **SPRINGFEST2026** - Moonlight Crystal ×10, Gold ×8,888
- **b8n2k5l** - Flawless Fluorite ×2, Rare Timber ×6, Stone ×10
- **m7r9q4a** - Gold ×10,000 (pair with SPRINGFEST2026 for bonus)

High-value bonus codes: `letsbuild`, `letsdressup`, `letsparty` for Wishing Stars + Festival Materials.

Expired codes (do not attempt):
- `p7a9k2m6r8`, `x8r2m9q5l7`, `q6p9m4a7k` - Expired March 31, 2025

## Events & quests
- **Winter Event Week 5 (FINAL WEEK):** Penguin story concludes with radish, winter food, and winter bird card collection missions. Sea Lantern event at the beach. Don't miss the finale!
- **Heartopia × My Little Pony Collaboration:** Official post states event runs from **February 14 to March 24, 2026** (currently active). Daily Tree of Harmony sensing delivers Exhibition Passes, Moonlight Crystals, and Badge Pull Tokens.
- **Penguin Gifting event:** ~14 days remaining with seven pastel presents; schedule nightly vendor visits at East Beach.
- **Moonlight Treasure Box:** Secret Recipes active—draw with Moonlight Crystals for outfit sets with a Moonlight Special guarantee within 12 draws.
- **Spring Gift redemption:** SPRINGFEST2026 code active until March 31, 2026 (10:59 UTC-5).

## Community resources
- **Bookmark Lunar Nook and Glyph Hub** for daily coordinate updates—they're the fastest sources for verified plot locations.
- **Heartopia Fess** provides quick Indonesian-language updates with screenshot confirmations.

## Last references
- @Glyph_Hub · Roaming Oak & Flourite (Mar 1, 2026) · https://x.com/Glyph_Hub/status/2027871760813854867
- @heartopiafess · Mar 1 fluorite & oak confirmation (Mar 1, 2026) · https://x.com/heartopiafess/status/2027911631653437831
- @LunarNook · Winter Event Week 5 Announcement (Feb 28, 2026) · https://x.com/LunarNook/status/2027862803789299718
- @Glyph_Hub · Doris & Meteor Shards Locations (Feb 28, 2026) · https://x.com/Glyph_Hub/status/2027718475196903759
- @Glyph_Hub · All 15 Pink Bubble Locations (Feb 28-Mar 7, 2026) · https://x.com/Glyph_Hub/status/2027527577527824628
- @LunarNook · Feb 28 Daily Coordinates + Meteor Shower (Feb 28, 2026) · https://x.com/LunarNook/status/2027651305099927738
